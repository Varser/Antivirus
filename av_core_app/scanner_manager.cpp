...
/*static*/ SCANNER_ID scanner_manager::create_scanner(const QString& file)
{
    ...
    QSharedPointer<scanner> t_scanner(new scanner(file), [](scanner* p){p->stop(); p->quit(); p->wait(); delete p;});
...
{
    auto current_id = ++scanner_manager::_id;
    QSharedPointer<scanner> t_scanner(new scanner(data), [](scanner* p){p->stop(); p->quit(); p->wait(); delete p;});
    _manager._scanners.insert(current_id, t_scanner);
    t_scanner->start();
    return current_id;
}
