...
void scanner::scan_file()
{
    ...
    qint64 num_iterations = (file_size/piece_size + (file_size%piece_size != 0));
    qDebug() << "File size is " << file_size << " Will be processed at " << num_iterations << " iterations";
    ...
    for (qint64 i = 0; i < num_iterations; ++i)
    {
        if (_stopped)
        {
            file.close();
        ...
        _percent = 100*(i + 1)/num_iterations;
